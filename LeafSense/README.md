...
| Part | Location | Role |
|------|----------|------|
| **Backend (API)** | `app.py` (project root) | Flask app: loads EfficientNet model, exposes `POST /predict`, serves built UI from `frontend/dist` |
| **Front end (UI)** | `frontend/` | React + Vite + TypeScript + shadcn/ui. Build output: `frontend/dist/` |
| **Legacy UI (fallback)** | `legacy_templates/` | HTML-only fallback if `frontend/dist` is missing |
| **Paper assets (docs only)** | `docs/paper-assets/` | Figures for the research write-up (not used at runtime) |
| **Project brief** | `docs/PROJECT_BRIEF_FOR_RESEARCH_PAPER.md` | Extended description for drafting the paper |

**Frontend env:** copy `frontend/.env.example` to `frontend/.env` for local dev (`VITE_API_BASE_URL` pointing at Flask). Production values stay out of git (see `.gitignore`).
...
Front end source: `frontend/src/lib/api.ts` (`predictImage()`).
...
2. In another terminal, start the React dev server:
   ...
   npm run dev
   ```
3. Open **http://localhost:5173** (Vite dev server). The UI calls the API at `http://localhost:5000` (set in `frontend/src/lib/api.ts` via `VITE_API_BASE_URL` or default).
...
The build installs Python deps (including torch) and builds the React app. At runtime, gunicorn runs Flask, which serves both the API and the built React files from `frontend/dist`.
...
- Deploy **only the frontend** (build `frontend` with `npm run build`, publish the `dist/` folder).
...
- **Backend:** Uses uploaded file (field `image`), saves to a temp file, runs prediction, returns JSON, then deletes the temp file. CORS enabled for dev. Serves React build from `frontend/dist` at `/` when present; fallback to `legacy_templates/index.html` if not built.
- **Front end:** Hero section no longer depends on a missing `hero-bg.jpg` (uses CSS gradient). Production build uses relative `/predict` when served from Flask (configure `VITE_API_BASE_URL` in `frontend/.env` for dev).
- **Legacy templates:** Kept for fallback only; primary UI is the React app in `frontend/`.
