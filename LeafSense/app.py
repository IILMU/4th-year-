...
# Initialize Flask app (legacy HTML lives in legacy_templates/ when React build is absent)
app = Flask(__name__, template_folder="legacy_templates")
...
# Front end: React app in frontend/. Build with: cd frontend && npm run build
...
def predict_route():
    ...
    # Front end sends the file with key "image" (see frontend/src/lib/api.ts)
...
def serve_spa(path):
    if not os.path.isdir(FRONTEND_DIST):
        return jsonify({"error": "Front end not built. Run: cd frontend && npm run build"}), 404
